...
-- 1. Denormalización: el cliente puede ver "Maria viene a tu mesa"
--    sin tener que leer la tabla `perfiles` (cuya RLS no le da acceso).
...
-- 2. Tabla de reseñas de la visita (estrellas + comentario opcional).
...
-- 3. RLS para que el cliente vea pagos/llamados de SU sesión por realtime.
...
-- 4. RLS para reviews (cliente insert/select de SU sesión, staff select).
...
-- 5. Habilitar Realtime para tablas que el cliente subscribe.
ALTER TABLE pagos REPLICA IDENTITY FULL;
ALTER PUBLICATION supabase_realtime ADD TABLE pagos;
ALTER PUBLICATION supabase_realtime ADD TABLE reviews;

-- 6. Fix check constraint de pagos.metodo (estaba limitado a "efectivo").
