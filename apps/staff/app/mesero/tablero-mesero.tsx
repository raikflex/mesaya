...
async function traerComandaCompleta(comandaId: string): Promise<ComandaListaMesero | null> {
  ...
  const { data } = await supabase
    .from('comandas')
    .select(`
      id, numero_diario, total, creada_en, mesero_atendiendo_id, origen,
...
function CabeceraSeccion({ titulo, descripcion, count, bgColor, fgColor }: { titulo: string; descripcion: string; count: number; bgColor: string; fgColor: string }) {
  return (
    <div className="mb-3 flex items-baseline justify-between gap-3">
      <div>
        <h2 className="font-[family-name:var(--font-display)] text-xl tracking-[-0.015em]" style={{ color: 'var(--color-ink)' }}>{titulo}</h2>
        <p className="text-[0.7rem] mt-0.5" style={{ color: 'var(--color-muted)' }}>{descripcion}</p>
      </div>
      <span className="text-xs uppercase tracking-[0.1em] px-2.5 py-1 rounded-full font-medium shrink-0" style={{ background: bgColor, color: fgColor }}>{count}</span>
    </div>
  );
}
  ...
  return (
    <CardBase esMio={esMio} esDeOtro={esDeOtro} colorMarca={colorMarca} pending={pending}>
      <header className="px-4 py-3 flex items-center justify-between gap-3 border-b" style={{ borderColor: 'var(--color-border)' }}>
        <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: 'var(--color-paper-deep)', color: 'var(--color-ink-soft)' }}>Mesa {llamado.mesaNumero}</span>
        <TiempoTranscurrido fecha={llamado.creadoEn} />
      </header>
      <div className="px-4 py-3">
        <p className="text-sm font-medium" style={{ color: 'var(--color-ink)' }}>{llamado.motivo === 'campana' ? 'Necesita ayuda' : 'Llamado del cliente'}</p>
        {llamado.nota ? (
          <div className="mt-2 rounded-[var(--radius-md)] border px-3 py-2" style={{ borderColor: '#fde68a', background: '#fefce8' }}>
            <p className="text-[0.65rem] uppercase tracking-[0.12em] mb-0.5" style={{ color: '#92400e' }}>El cliente dice</p>
      ...
      <CardBase esMio={esMio} esDeOtro={esDeOtro} colorMarca={colorMarca} pending={pending}>
        <header className="px-4 py-3 flex items-center justify-between gap-3 border-b" style={{ borderColor: 'var(--color-border)' }}>
          <div className="flex items-center gap-3 min-w-0">
            <span className="font-[family-name:var(--font-display)] text-base tabular-nums" style={{ color: 'var(--color-ink)' }}>#{comanda.numeroDiario.toString().padStart(3, '0')}</span>
            {esDomicilio ? (
              <span className="text-xs px-2 py-0.5 rounded-full font-medium" style={{ background: comanda.origen === 'domicilio' ? '#dbeafe' : '#dcfce7', color: comanda.origen === 'domicilio' ? '#1e40af' : '#166534' }}>
                {comanda.origen === 'domicilio' ? 'Domicilio' : 'Para recoger'}
              </span>
            ) : (
              <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: 'var(--color-paper-deep)', color: 'var(--color-ink-soft)' }}>Mesa {comanda.mesaNumero}</span>
            )}
          </div>
          <TiempoTranscurrido fecha={comanda.creadaEn} />
        </header>
        <div className="px-4 py-3">
          <p className="text-[0.7rem] uppercase tracking-[0.1em] mb-2" style={{ color: 'var(--color-muted)' }}>{comanda.clienteNombre}</p>
          ...
          {comanda.entrega ? (
            <div className="mt-3 pt-3 border-t rounded-[var(--radius-md)] p-2.5" style={{ borderColor: 'var(--color-border)', background: 'var(--color-paper)' }}>
              <p className="text-[0.65rem] uppercase tracking-[0.12em] mb-1.5" style={{ color: 'var(--color-muted)' }}>
              ...
              {comanda.entrega.notasEntrega ? <p className="text-xs mt-0.5 italic" style={{ color: 'var(--color-ink-soft)' }}>{comanda.entrega.notasEntrega}</p> : null}
              <button
                type="button"
      ...
      <CardBase esMio={esMio} esDeOtro={esDeOtro} colorMarca={colorMarca} pending={pending}>
        <header className="px-4 py-3 flex items-center justify-between gap-3 border-b" style={{ borderColor: 'var(--color-border)' }}>
          <div className="flex items-center gap-2 min-w-0 flex-wrap">
            <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: 'var(--color-paper-deep)', color: 'var(--color-ink-soft)' }}>Mesa {pago.mesaNumero}</span>
            <span className="text-[0.7rem]" style={{ color: 'var(--color-muted)' }}>{pago.cantidadComandas} pedido{pago.cantidadComandas === 1 ? '' : 's'}</span>
            {formaPago ? <span className="text-[0.65rem] uppercase tracking-[0.05em] px-1.5 py-0.5 rounded font-medium" style={{ background: formaPago.bg, color: formaPago.fg }}>{formaPago.label}</span> : null}
            {tieneFactura ? <span className="text-[0.65rem] uppercase tracking-[0.1em] px-1.5 py-0.5 rounded font-medium" style={{ background: '#dbeafe', color: '#1e40af' }}>Pide factura</span> : null}
          </div>
...
function ModalCobrar({ pago, colorMarca, onCerrar }: { pago: PagoMesero; colorMarca: string; onCerrar: () => void }) {
  ...
  function confirmar() {
    ...
    startTransition(async () => {
      const r = await confirmarPago({ llamadoId: pago.id, metodoConfirmado: metodo, conPropina });
    ...
    <div className="fixed inset-0 z-50 grid place-items-center px-4 py-8" style={{ background: 'rgba(26, 24, 20, 0.6)' }} onClick={onCerrar}>
      <div className="w-full max-w-md rounded-[var(--radius-lg)] bg-white overflow-hidden" style={{ maxHeight: '90vh', overflowY: 'auto' }} onClick={(e) => e.stopPropagation()}>
        <header className="px-5 py-4 border-b" style={{ borderColor: 'var(--color-border)' }}>
          <p className="text-[0.65rem] uppercase tracking-[0.14em]" style={{ color: 'var(--color-muted)' }}>Cobrar Mesa {pago.mesaNumero}</p>
        ...
        <div className="px-5 py-4 space-y-4">
                ...
                {pago.comandas.map((c, idxC) => (
                  <li key={`${c.numeroDiario}-${idxC}`} className="px-4 py-3">
                    <div className="flex items-baseline justify-between mb-2">
                      <p className="font-[family-name:var(--font-display)] text-sm tabular-nums" style={{ color: 'var(--color-ink)' }}>Comanda #{c.numeroDiario.toString().padStart(3, '0')}</p>
                      <span className="font-[family-name:var(--font-mono)] text-xs" style={{ color: 'var(--color-ink-soft)' }}>${c.total.toLocaleString('es-CO')}</span>
                    </div>
                    <ul className="space-y-1">
                      {c.items.map((it, idxI) => (
                        <li key={idxI} className="text-sm">
                          ...
                          {it.nota ? <p className="text-[0.7rem] ml-9 italic mt-0.5" style={{ color: 'var(--color-muted)' }}>"{it.nota}"</p> : null}
                        </li>
                      ))}
                    </ul>
                  </li>
          ...
          {pago.docNumero ? (
            <div className="rounded-[var(--radius-md)] border p-4" style={{ borderColor: '#3b82f6', background: '#eff6ff' }}>
              <p className="text-[0.65rem] uppercase tracking-[0.14em] mb-2" style={{ color: '#1e40af' }}>El cliente pidio factura</p>
              <dl className="space-y-1 text-sm">
                <div className="flex items-baseline gap-2"><dt className="text-[0.7rem] uppercase tracking-[0.1em] w-20 shrink-0" style={{ color: '#1e40af' }}>Tipo</dt><dd className="font-[family-name:var(--font-mono)]" style={{ color: 'var(--color-ink)' }}>{pago.docTipo}</dd></div>
                <div className="flex items-baseline gap-2"><dt className="text-[0.7rem] uppercase tracking-[0.1em] w-20 shrink-0" style={{ color: '#1e40af' }}>Numero</dt><dd className="font-[family-name:var(--font-mono)] select-all" style={{ color: 'var(--color-ink)' }}>{pago.docNumero}</dd></div>
                <div className="flex items-baseline gap-2"><dt className="text-[0.7rem] uppercase tracking-[0.1em] w-20 shrink-0" style={{ color: '#1e40af' }}>Nombre</dt><dd className="select-all" style={{ color: 'var(--color-ink)' }}>{pago.docNombre}</dd></div>
              </dl>
            </div>
          ) : null}
          ...
          <div className="rounded-[var(--radius-md)] border p-4" style={{ borderColor: 'var(--color-border)', background: 'var(--color-paper)' }}>
            ...
            <label className="flex items-center justify-between gap-3 py-2 cursor-pointer select-none">
              <span className="text-sm" style={{ color: 'var(--color-ink-soft)' }}>Propina (10%)</span>
              <div className="flex items-center gap-2">
                {conPropina ? <span className="text-sm font-[family-name:var(--font-mono)]" style={{ color: 'var(--color-ink)' }}>${propina.toLocaleString('es-CO')}</span> : null}
                <button type="button" role="switch" aria-checked={conPropina} onClick={() => setConPropina((v) => !v)} className="relative h-6 w-11 rounded-full transition-colors" style={{ background: conPropina ? colorMarca : 'var(--color-paper-deep)', border: `1px solid ${conPropina ? colorMarca : 'var(--color-border-strong)'}` }}>
                  <span className="absolute top-0.5 left-0.5 size-4 rounded-full bg-white shadow transition-transform" style={{ transform: conPropina ? 'translateX(20px)' : 'translateX(0)' }} />
                </button>
              </div>
            </label>
            <div className="border-t pt-3 mt-2 flex items-center justify-between" style={{ borderColor: 'var(--color-border)' }}>
              <span className="text-base font-medium" style={{ color: 'var(--color-ink)' }}>Total</span>
              <span className="font-[family-name:var(--font-display)] text-2xl" style={{ color: 'var(--color-ink)' }}>${total.toLocaleString('es-CO')}</span>
            </div>
          </div>

          <div>
            <p className="text-xs uppercase tracking-[0.14em] mb-2" style={{ color: 'var(--color-muted)' }}>Metodo de pago</p>
            <div className="grid grid-cols-2 gap-2">
              {([{ v: 'efectivo', l: 'Efectivo' }, { v: 'tarjeta', l: 'Tarjeta' }, { v: 'transferencia', l: 'Transferencia' }, { v: 'no_seguro', l: 'Otro' }] as const).map((opt) => (
                <button key={opt.v} type="button" onClick={() => setMetodo(opt.v)} className="h-10 rounded-[var(--radius-md)] border text-sm transition-colors" style={{ borderColor: metodo === opt.v ? colorMarca : 'var(--color-border-strong)', borderWidth: metodo === opt.v ? 1.5 : 1, background: metodo === opt.v ? 'var(--color-paper)' : 'white', color: metodo === opt.v ? colorMarca : 'var(--color-ink)' }}>{opt.l}</button>
              ))}
            </div>
          </div>

          {error ? <p role="alert" className="text-xs text-center" style={{ color: 'var(--color-danger)' }}>{error}</p> : null}
          <p className="text-[0.7rem] text-center px-2" style={{ color: 'var(--color-muted)' }}>Al confirmar, la mesa se cierra y todas las comandas pendientes se marcan como entregadas.</p>
        </div>
        <footer className="px-5 py-4 border-t flex items-center gap-3" style={{ borderColor: 'var(--color-border)', background: 'var(--color-paper)' }}>
          <button type="button" onClick={onCerrar} disabled={pending} className="flex-1 h-11 rounded-[var(--radius-md)] text-sm border" style={{ borderColor: 'var(--color-border-strong)', color: 'var(--color-ink)', background: 'white' }}>Cancelar</button>
          <button type="button" onClick={confirmar} disabled={pending} className="flex-1 h-11 rounded-[var(--radius-md)] text-sm font-medium disabled:opacity-50" style={{ background: colorMarca, color: 'white' }}>{pending ? 'Cobrando...' : 'Confirmar y cerrar'}</button>
        </footer>
      </div>
    </div>
  );
...
function FooterAcciones({ esMio, esDeOtro, colorMarca, pending, onTomar, onLiberar, onAccionPrimaria, textoAccionPrimaria, infoExtra }: { esMio: boolean; esDeOtro: boolean; colorMarca: string; pending: boolean; onTomar: () => void; onLiberar: () => void; onAccionPrimaria: () => void; textoAccionPrimaria: string; infoExtra?: string }) {
  return (
    <footer className="px-4 py-2.5 border-t flex items-center justify-between gap-3" style={{ borderColor: 'var(--color-border)', background: 'var(--color-paper)' }}>
      {infoExtra ? <span className="text-[0.7rem]" style={{ color: 'var(--color-muted)' }}>{infoExtra}</span> : <span />}
...
function SeccionVacia({ mensaje, borderColor }: { mensaje: string; borderColor: string }) {
  return (
    <div className="rounded-[var(--radius-lg)] border-2 border-dashed py-10 px-4 text-center" style={{ borderColor }}>
      <p className="text-xs italic" style={{ color: 'var(--color-muted)' }}>{mensaje}</p>
    </div>
  );
}
...
function TiempoTranscurrido({ fecha }: { fecha: string }) {
  ...
  return <span className="text-[0.7rem] tabular-nums" style={{ color: 'var(--color-muted)' }}>{texto}</span>;
}
